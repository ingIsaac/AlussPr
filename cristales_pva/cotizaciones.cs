                ...
                if (sql.findSQLValue("folio", "folio", "cotizaciones", constants.folio_abierto.ToString()) == false)
                {
                    if (sql.isFolioExist(folio) == false)
                    {
                        if (textbox.Text != "")
                        {
                            ...
                            sql.insertCotizacion(folio, textbox.Text, constants.user, DateTime.Today.ToString("dd/MM/yyyy") + " -", textbox2.Text, constants.desc_cotizacion, constants.utilidad_cotizacion, constants.org_name, constants.iva_desglosado, "Sin Registro", constants.tc, constants.serializeSubfolio(), constants.precio_especial_desc);
